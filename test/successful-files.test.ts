...
describe('runs transform', () => {
  ...
  test('async', async () => {
    const result = await cosmiconfig('cosmiconfig-tests', { transform }).load(
      configPath,
    );
    checkResult(result);
  });
  ...
  test('sync', () => {
    const result = cosmiconfig('cosmiconfig-tests', { transform }).loadSync(
      configPath,
    );
    checkResult(result);
  });
...
describe('does not swallow transform errors', () => {
  ...
  test('async', async () => {
    ...
    try {
      await cosmiconfig('cosmiconfig-tests', { transform }).load(configPath);
    } catch (error) {
      checkError(error);
    }
  });
  ...
  test('sync', () => {
    expect.hasAssertions();
    try {
      cosmiconfig('cosmiconfig-tests', { transform }).loadSync(configPath);
    } catch (error) {
      checkError(error);
    }
  });
...
describe('loads defined JS config relative path', () => {
  ...
  test('async', async () => {
    const result = await cosmiconfig('cosmiconfig-tests').load(relativeFile);
    checkResult(result);
  });

  test('sync', () => {
    const result = cosmiconfig('cosmiconfig-tests').loadSync(relativeFile);
    checkResult(result);
  });
});
