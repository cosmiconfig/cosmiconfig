  ...
  function searchDirectory(directory, splitSearchPath) {
    ...
    const result = Promise.resolve()
      ...
      .then((result) => {
        ...
        const nextDirectory = (function () {
          if (!splitSearchPath) splitSearchPath = splitPath(directory);
          if (splitSearchPath.length > 1) {
            splitSearchPath.pop();
            return joinPath(splitSearchPath);
          }
        })();

        if (!nextDirectory || directory === options.stopDir) return null;

        return searchDirectory(nextDirectory, splitSearchPath);
      })
      .then(transform);
