...
module.exports = function (options) {
  ...
  function load(searchPath, configPath) {
    ...
    return isDirectory(absoluteSearchPath)
      .then((searchPathIsDirectory) => {
        ...
        return getDirectoryCache(directory) || searchDirectory(splitPath(directory));
      });
  }

  function searchDirectory(splitSearchPath) {
    const directory = joinPath(splitSearchPath);
    const result = getDirectoryCache(directory) || Promise.resolve()
      .then(() => {
      ...
      .then((result) => {
        ...
        if (directory === options.stopDir || !splitSearchPath.pop()) return null;
        return searchDirectory(splitSearchPath);
      })
      .then(transform);
  ...
  function getDirectoryCache(directory){
