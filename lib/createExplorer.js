...
function resolveFrom(moduleId, prefix, fromDir, fromFile) {
  if (fromDir) {
...
function getModulePath(moduleId, fromFile, prefix) {
  ...
  return isFile(AbsPath).catch(function (){
    ...
    if (isFile) {
      ...
      return resolveFrom(moduleId, prefix, null, fromFile) || resolveFrom(moduleId, prefix, process.cwd()) || resolveFrom(moduleId, prefix);
    }
  });
}
    ...
    if (config && props) {
      return mapArr(props, function (prop){
        ...
        return mapArrStr(config[prop], function (moduleId) {
          return getModulePath(moduleId, filepath, prefix).then(function (modulePath) {
            if (modulePath) {
