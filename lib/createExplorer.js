...
module.exports = function (options) {
  ...
  const searchPathCache = (options.cache) ? new Map() : null;
  ...
  function load(searchPath, configPath) {
    ...
    if (searchPathCache && searchPathCache.has(absoluteSearchPath)) {
      return searchPathCache.get(absoluteSearchPath);
    }

    const result = isDirectory(absoluteSearchPath)
    ...
    if (searchPathCache) searchPathCache.set(absoluteSearchPath, result);
    return result;
  }
