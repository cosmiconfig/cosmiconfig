...
describe('finds package.json prop in second searched dir', () => {
  beforeEach(() => {
    temp.createJsonFile('a/b/c/d/e/package.json', {
      author: 'Todd',
      foo: { found: true },
    });
  });
...
describe('finds package.json with nested packageProp in second searched dir', () => {
  beforeEach(() => {
    ...
    temp.createJsonFile('a/b/c/d/e/f/package.json', {
      author: 'Todd',
      configs: { notYourPkg: { yes: 'ofcourse' } },
    });
    temp.createJsonFile('a/b/c/d/e/package.json', {
      author: 'Todd',
      configs: { pkg: { please: 'no' } },
    });
  });
...
describe('finds ESM foo.config.js file in first searched dir', () => {
  ...
  test('async', async () => {
    ...
    temp.createJsonFile(`${dirPrefix}/a/b/c/d/e/f/package.json`, {
      type: 'module',
    });
...
describe("finds foorc.json file in first searched dir's .config subdir", () => {
  beforeEach(() => {
    temp.createJsonFile('a/b/c/d/e/f/.config/foorc.json', { found: true });
  });
...
describe('finds package.json in second dir searched, with alternate names', () => {
  beforeEach(() => {
    temp.createJsonFile('a/b/c/d/e/package.json', { heeha: { found: true } });
  });
...
describe('finds .foorc.json in second searched dir', () => {
  beforeEach(() => {
    temp.createJsonFile('a/b/c/d/e/.foorc.json', { found: true });
  });
...
describe('finds .foorc.json in first searched dir', () => {
  beforeEach(() => {
    temp.createJsonFile('a/b/c/d/e/f/.foorc.json', { found: true });
  });
...
describe('searchPlaces can include subdirectories', () => {
  beforeEach(() => {
    temp.createJsonFile('a/b/c/d/e/.config/.foorc.json', { found: true });
  });
...
describe('directories with the same name as a search place are not treated as files', () => {
  beforeEach(() => {
    temp.createJsonFile('a/.foorc.json', { found: true });
